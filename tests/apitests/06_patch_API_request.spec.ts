...
2) Create a token
3) Patch update the booking details using bookingId and token (PATCH API request)
*/
...
test('Patch update Booking', async ({ request }) => {
...
//Step 3: Patch update the booking details using bookingId and token (PATCH API request)
    const requestbody3 = {
        firstname : "Pooja",
        lastname : "Prasanna",
    }
    const repsonse3 = await request.patch(`https://restful-booker.herokuapp.com/booking/${bookingID}`,
        {
            data: requestbody3,
